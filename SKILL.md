...
써봤으면 별점(N/5), 아직이면 점수 비우고 '찜'으로 올려도 돼요.
...
스킬봇 `/스킬등록` 모달에 **그대로 넣을 초안**을 만든다. 이미 아는 정보 자동 채움. 부족한 거 1개만 짧게 묻기:
> "한 번 써보고 어땠어요? 1줄로. (시간 절약 / 단점 / 의외 발견)"
...
출력 끝에:
```
✅ 채널에서 /스킬등록 치고 위 내용 그대로 넣으면 돼요.
   결과물(영상·이미지)은 올린 뒤 그 스레드에 댓글로 첨부하면 됩니다.
```
